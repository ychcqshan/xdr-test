...
public interface HostAssetRecordMapper extends BaseMapper<HostAssetRecord> {
    ...
    @Update("UPDATE host_asset_record SET status = 'INACTIVE' WHERE agent_id = #{agentId} AND status = 'ACTIVE' AND last_updated < #{threshold}")
    int markInactiveBefore(@Param("agentId") String agentId, @Param("threshold") LocalDateTime threshold);
