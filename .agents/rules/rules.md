...
- **上下文扫描**: 在开始任何工作前，**必须**先扫描 `.context/memories.md`，确保遵循项目长期沉淀的经验与规避已知问题。
...
3. **记忆提纯 (Memory Purification)**: 
   - 任务完成后，若 `error_journal.md` 中记录的教训具有普遍意义，**必须**将其提炼成“避坑指南”并更新至 `.context/memories.md`。
   - 定期清理 `error_journal.md` 冗余记录，确保 AI 初始加载时只面对“提纯后的真理”。

4. **提取规则 (Rule Extraction)**: 
   - 必须将通用的教训提取并记录，确保在未来的会话中永不再犯同类错误。
