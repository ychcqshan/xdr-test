...
| 基线类型 | 类型标识 | 基线项字段 | 比对键 |
|---------|---------|-----------|--------|
| 进程基线 | PROCESS | 进程名、可执行路径、命令行Hash、运行用户 | 进程名 + 二进制路径 |
| 端口基线 | PORT | 端口号、协议(TCP/UDP)、绑定IP、**关联进程名** | 端口号 + 协议 + 进程名 |
...
#### 4.1.5 资产风险评分规则 (Scoring Engine)
- **核心算法**：`FinalScore = 100 - (AlertWeights + BaselineDeviations + StatusPenalty)`
- **扣分细则**：
  - **Critical Alert**: -40
  - **High Alert**: -20
  - **Medium Alert**: -10
  - **Baseline Deviation**: -15 每项
  - **Offline Status**: -5
...
| F-ASSET-003 | 多条件搜索 | 按主机名/IP/OS/状态/风险等级/集团/二-四级单位/部门/责任人组合搜索 | el-form 筛选栏 (13px 标签) |
...
| 功能编号 | 功能名称 | 功能描述 | UI组件 |
|---------|---------|---------|--------|
| F-SYS-001 | 用户管理 | 管理后台用户账号 (支持四级单位/岗位分配) | el-table + el-dialog |
