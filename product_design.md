...
| 功能编号 | 功能名称 | 功能描述 | 采集内容 | 默认频率 |
|---------|---------|---------|---------|----------|
| A-COLL-001 | 进程采集 | 采集系统运行进程的完整信息 | PID、进程名、可执行路径、命令行参数、所属用户、文件Hash(MD5/SHA256)、父进程PID、启动时间、CPU/内存占用 | 60s |
| A-COLL-002 | 网络连接采集 | 采集系统所有活动网络连接 | 本地IP:端口、远端IP:端口、协议(TCP/UDP)、连接状态、关联进程PID、流量统计(字节数) | 60s |
| A-COLL-003 | USB设备采集 | 监控USB设备的插拔事件 | 厂商ID、产品ID、设备序列号、挂载点、设备类型(存储/HID/其他)、插入/拔出时间 | 实时 |
| A-COLL-004 | 登录事件采集 | 采集系统登录/注销事件 | 用户名、来源IP(远程登录)、登录类型(本地/远程/服务)、登录时间、登出时间、成功/失败状态 | 实时 |
| A-COLL-005 | 资产信息采集 | 采集终端硬件和软件资产信息 | 主机名、OS版本、CPU型号/架构、内存总量/可用、磁盘总量/可用、已安装软件列表(名称/版本/安装日期)、网卡MAC/IP | 300s |
...
| 功能编号 | 功能名称 | 功能描述 | 检测方法 |
|---------|---------|---------|----------|
| A-DET-001 | 勒索软件检测 | 监控文件系统异常加密行为 | watchdog监控文件后缀批量修改(.lock/.crypt/.encrypted等)，1分钟内>N次触发告警 |
| A-DET-002 | 横向移动检测 | 识别网络内横向扩散行为 | 监控PsExec/WMI/SMB远程执行工具的进程创建和网络连接 |
| A-DET-003 | 无文件攻击检测 | 识别内存态恶意代码执行 | 监控PowerShell/WMI脚本执行，检测可疑参数(-enc/-nop/-bypass)和内存注入行为 |
| A-DET-004 | YARA规则匹配 | 基于特征签名匹配已知恶意软件 | yara-python加载规则文件，对新增进程和文件进行实时扫描 |
...
|----|------|
| `psutil` | 进程/网络/系统信息采集 |
| `watchdog` | 文件系统变更监控(勒索软件检测) |
| `pyudev` / `wmi` | USB设备监控(Linux/Windows) |
| `requests` / `httpx` | HTTP通信 + mTLS |
| `cryptography` | AES-256/RSA加密、证书管理 |
| `sqlite3` | 本地缓存(内置标准库) |
| `schedule` / `APScheduler` | 定时任务调度 |
